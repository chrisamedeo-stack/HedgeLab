...
import { suggestFuturesMonth, getValidDeliveryMonths } from "@/lib/commodity-utils";
...
function OffsetForm({
  ...
  const lots = Math.round(bu / config.contractSizeBu);
  ...
  async function handleSubmit() {
    if (lots <= 0 || bu > availableBu) {
      toast.toast(`Bushels must be ${config.contractSizeBu}\u2013${fmtBu(availableBu)}`, "error");
      return;
    }
    if (isNaN(exit)) {
    ...
    <div className="bg-input-bg/70 border border-b-input rounded-lg p-4 mt-2">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-3">
        <div className="flex flex-col gap-1">
          ...
          <input type="number" step={config.contractSizeBu} min={config.contractSizeBu} value={bushels} onChange={(e) => setBushels(e.target.value)}
...
function NewPurchaseForm({
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      await api.post(`${apiBase}/contracts`, {
        siteCode: site,
...
function EFPForm({
  ...
  const { config, apiBase } = useCommodity();
  const toast = useToast();
  const validMonths = getValidDeliveryMonths(config, allocation.futuresMonth);
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-input-bg/50">
            ...
            <SortableHeader label="FM" sortKey="futuresMonth" activeKey={allocSort.key} activeDir={allocSort.dir} onToggle={(k) => toggleAllocSort(k as AllocSortKey)} />
            ...
            return (
              <Fragment key={a.allocationId}>
                <tr className="border-t border-b-default hover:bg-row-hover transition-colors">
                  ...
                  <td className="px-4 py-3 text-secondary">{fmtVol(a.allocatedBushels, config.bushelsPerMt)}</td>
                  <td className="px-4 py-3 text-secondary font-semibold">{fmtVol(a.openAllocatedLots * config.contractSizeBu, config.bushelsPerMt)}</td>
                  ...
                  <tr className="border-t border-b-default animate-slide-down">
                    <td colSpan={12} className="px-4 py-3">
                      <OffsetForm
                        ...
                        availableBu={a.openAllocatedLots * config.contractSizeBu}
                        endpoint={`${apiBase}/hedges/allocations/${a.allocationId}/offset`}
                        onDone={handleDone}
          ...
          <tbody>
            ...
            {positions.map((p) => (
              <tr key={p.contractId} className="border-t border-b-default hover:bg-row-hover transition-colors">
                ...
                <td className="px-4 py-3 text-secondary">{fmtVol(p.committedMt, config.bushelsPerMt, "MT")}</td>
                <td className="px-4 py-3">
                  {p.efpExecuted ? (
      ...
      {positions.length > 0 && blended.blendedPerBu != null && (
        <div className="px-5 py-3 border-t border-b-default bg-input-bg/30">
          <span className="text-xs text-muted">
            ...
            {blended.hedgedBu > 0 && <span> (hedged: ${blended.hedgedPerBu?.toFixed(4)}/bu x {fmtVol(blended.hedgedBu, config.bushelsPerMt)} bu</span>}
            {blended.marketBu > 0 && <span>{blended.hedgedBu > 0 ? " + " : " ("}market: ${blended.marketPerBu?.toFixed(4)}/bu x {fmtVol(blended.marketBu, config.bushelsPerMt)} bu</span>}
    ...
    <div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-input-bg/50">
              {["Ticket", "Dir", "FM", "Lots", "Bu", "Trade Date", "Price $/bu", "P&L $/bu", "P&L $"].map((h) => (
          ...
          <tbody>
            ...
            {locked.map((l) => {
              const bu = l.lots * config.contractSizeBu;
              ...
              return (
                <Fragment key={l.efpTicketId}>
                  ...
                  <tr className="border-t border-b-default hover:bg-row-hover">
                    ...
                    <td className="px-3 py-2 text-secondary">{fmtVol(bu, config.bushelsPerMt)}</td>
    ...
    <div>
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-input-bg/50">
              {["Trade", "Dir", "FM", "Lots", "Bu", "Date", "Price $/bu", "P&L $/bu", "P&L $"].map((h) => (
                <th key={h} className="px-3 py-3 text-left text-xs font-semibold text-faint uppercase tracking-wider whitespace-nowrap">{h}</th>
              ))}
            </tr>
          </thead>
              ...
              return (
                <Fragment key={o.offsetId}>
                  ...
                  <tr className="border-t border-b-default hover:bg-row-hover">
                    ...
                    <td className="px-3 py-2 text-secondary">{fmtVol(o.bushels, config.bushelsPerMt)}</td>
...
function MonthCoverageSummary({
  ...
  const summary = useMemo(() => {
    ...
    const futuresMonth = suggestFuturesMonth(config, budgetMonth);
  ...
  }, [budgetMonth, budgetLines, siteFilter, settles, config, BUSHELS_PER_MT]);
      ...
      <div className="grid grid-cols-4 gap-4">
        <div>
          ...
          <p className="text-lg font-bold tabular-nums text-primary">
            {fmtVol(summary.budgetMt, config.bushelsPerMt, "MT")} <span className="text-xs text-faint font-normal">bu</span>
          </p>
        </div>
        <div>
          <p className="text-xs text-faint uppercase tracking-wider mb-1">Hedged</p>
          <p className="text-lg font-bold tabular-nums text-primary">
            {fmtVol(summary.hedgedMt, config.bushelsPerMt, "MT")} <span className="text-xs text-faint font-normal">bu</span>
          </p>
...
export default function SitesPage() {
  ...
  const { positions, isLoading, error, mutate } = usePositions(slug, book);
  const { sites } = useSites(slug);
  ...
  const { budget: budgetLines } = useBudget(slug, siteFilter || undefined, currentFiscalYear(fyStartMonth) || undefined);
      ...
      <div className="flex items-center gap-4 bg-surface/50 border border-b-default rounded-lg px-5 py-3">
        ...
        {budgetMonth && (
          <span className="text-xs text-ph">
            Futures: <span className="text-action font-mono">{suggestFuturesMonth(config, budgetMonth)}</span>
          </span>
        )}
      </div>
      ...
      <div className="bg-surface border border-b-default rounded-lg overflow-hidden">
        ...
        >
          ...
          <span className="text-xs text-faint">
            ...
            {fmtVol(filteredAllocations.reduce((s, a) => s + a.openAllocatedLots * config.contractSizeBu, 0), config.bushelsPerMt)} bu
          </span>
        ...
        >
          ...
          <span className="text-xs text-faint">
            ...
            {fmtVol(filteredOffsets.reduce((s, o) => s + o.bushels, 0), config.bushelsPerMt)} bu
          </span>
        ...
        <div className="px-5 py-4 border-b border-b-default flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-faint mt-0.5">
              ...
              {fmtVol(filteredPhysical.reduce((s, p) => s + p.committedMt, 0), config.bushelsPerMt, "MT")} bu committed
            </p>
          </div>
          <button
