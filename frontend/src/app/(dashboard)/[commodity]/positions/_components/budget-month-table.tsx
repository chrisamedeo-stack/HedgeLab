...
import { fmtBu, fmtPerBu, fmtPnl, pnlColor, inputCls, btnPrimary, btnCancel } from "@/lib/corn-format";
...
export function BudgetMonthTable({
  ...
  const groups: BudgetMonthGroup[] = useMemo(() => {
    ...
    return Array.from(map.entries())
      .map(([budgetMonth, { monthOnly, siteAssigned }]) => {
        ...
        const totalBu = totalLots * config.contractSizeBu;
  ...
  async function handleAllocate(budgetMonth: string) {
    ...
    if (lots <= 0) { toast.toast(`Min ${config.contractSizeBu.toLocaleString()} bu (1 lot)`, "error"); return; }
    setAllocSaving(true);
    try {
          ...
          <Fragment key={g.budgetMonth}>
            ...
            >
              ...
              <div className="flex flex-col">
                ...
                <span className="text-sm text-secondary tabular-nums">{fmtVol(g.totalBu, config.bushelsPerMt)}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] uppercase text-ph leading-tight">Alloc Bu</span>
                <span className="text-sm text-secondary tabular-nums">{fmtVol(g.siteAssigned.reduce((s, a) => s + a.allocatedBushels, 0), config.bushelsPerMt)}</span>
              </div>
              <div className="flex flex-col">
                <span className="text-[10px] uppercase text-ph leading-tight">Unassigned Bu</span>
                <span className="text-sm text-warning tabular-nums font-semibold">{fmtVol(g.monthOnly.reduce((s, a) => s + a.allocatedBushels, 0), config.bushelsPerMt)}</span>
              </div>
              <div className="flex flex-col">
                      ...
                      return (
                        <Fragment key={`mo-${a.allocationId}`}>
                          <tr className="border-t border-b-default hover:bg-row-hover transition-colors bg-warning-5">
                            ...
                            <td className="px-4 py-2 text-secondary tabular-nums">{fmtVol(a.allocatedBushels, config.bushelsPerMt)}</td>
