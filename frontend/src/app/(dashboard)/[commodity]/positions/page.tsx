...
export default function PositionsPage() {
  ...
  const { hedges, mutate: hedgesMutate } = useHedgesByBook(slug, book);
  ...
  async function handleDeleteConfirm() {
    ...
    try {
      await api.delete(`${apiBase}/hedges/${deleteTarget}`);
  ...
  async function handleUndoConfirm() {
    ...
    setUndoLoading(true);
    try {
      await api.delete(`${apiBase}/hedges/allocations/${undoTarget.id}`);
          ...
          {can("publish-settle") && (
            <button
              onClick={async () => {
                setRefreshing(true);
                try {
                  const res = await api.post<{ status: string; price?: number; months?: number; reason?: string }>(`${apiBase}/positions/refresh-prices`, {});
                  if (res.status === "ok") {
