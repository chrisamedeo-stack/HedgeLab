...
let _rowKey = 1;
function makeRow(futuresMonth?: string, defaultMonth?: string): HedgeLineRow {
  return { key: _rowKey++, futuresMonth: futuresMonth ?? defaultMonth ?? "", lots: "", pricePerBushel: "", notes: "" };
}
...
function AllocationPanel({
  ...
  const { allocations, isLoading, mutate } = useHedgeAllocations(slug, trade.id);
  ...
  async function handleMonthAllocate(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await api.post(`${apiBase}/hedges/${trade.id}/allocations`, {
        siteCode: null,
  ...
  async function handleAssignSite(monthAllocId: number) {
    ...
    setSubmitting(true);
    try {
      await api.post(`${apiBase}/hedges/allocations/${monthAllocId}/assign-site`, {
  ...
  async function handleDelete(id: number) {
    try {
      await api.delete(`${apiBase}/hedges/allocations/${id}`);
...
function HedgeTable({ book, showForm, setShowForm }: { book: "CANADA" | "US"; showForm: boolean; setShowForm: (v: boolean) => void }) {
  const { slug, config, apiBase } = useCommodity();
  const FUTURES_MONTHS = useMemo(() => generateFuturesMonths(config), [config]);
  const BUSHELS_PER_LOT = config.contractSizeBu;
  const BUSHELS_PER_MT = config.bushelsPerMt;
  const { hedges, isLoading, mutate } = useHedgesByBook(slug, book);
  const { toast } = useToast();
  const [submitting, setSubmitting]     = useState(false);
  ...
  function addRow() {
    ...
    const lastIdx = FUTURES_MONTHS.indexOf(lastMonth);
    const nextMonth = lastIdx >= 0 && lastIdx + 1 < FUTURES_MONTHS.length ? FUTURES_MONTHS[lastIdx + 1] : undefined;
  ...
  async function handleBulkSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (payloads.length === 1) {
        await api.post(`${apiBase}/hedges`, payloads[0]);
      } else {
        await api.post(`${apiBase}/hedges/bulk`, payloads);
      }
  ...
  async function handleEditSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await api.put(`${apiBase}/hedges/${editing.id}`, {
  ...
  async function handleDelete(h: HedgeTradeResponse) {
    try {
      await api.delete(`${apiBase}/hedges/${h.id}`);
                    ...
                    <tr key={r.key} className={cn("hover:bg-row-hover", incomplete && "bg-destructive-10/50")}>
                      <td className="px-3 py-1.5">
                        <select value={r.futuresMonth} onChange={(e) => updateRow(r.key, "futuresMonth", e.target.value)}
                          ...
                          {FUTURES_MONTHS.map((m) => <option key={m} className="bg-input-bg">{m}</option>)}
                        </select>
                      </td>
                      <td className="px-3 py-1.5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
            <div className="space-y-1">
              ...
              <select
                ...
                required
              >
                {FUTURES_MONTHS.map((m) => <option key={m}>{m}</option>)}
              </select>
            </div>
            <div className="space-y-1">
              <label className="text-xs text-muted">Lots ({BUSHELS_PER_LOT.toLocaleString()} bu each)</label>
              <input
                type="number" min="1"
                className="w-full bg-input-bg border border-b-input text-primary rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-focus placeholder:text-ph"
                placeholder="e.g. 40"
