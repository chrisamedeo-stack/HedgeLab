...
import { suggestFuturesMonth } from "@/lib/commodity-utils";
import { useToast } from "@/contexts/ToastContext";
import { btnPrimary, btnCancel } from "@/lib/corn-format";
import { getCommodityOptions, ComponentRow, fmtPrice } from "./shared";
...
export function FiscalYearGrid({ onSaved, onCancel, fyStartMonth = 7 }: { onSaved: () => void; onCancel: () => void; fyStartMonth?: number }) {
  const { slug, config: commodityConfig, apiBase } = useCommodity();
  const { sites } = useSites(slug);
  const { toast } = useToast();
  const [submitting, setSubmitting] = useState(false);
  const [config, setConfig] = useState({ siteCode: "", commodityCode: `${commodityConfig.code}-${commodityConfig.futuresPrefix}`, fiscalYear: availableFiscalYears(fyStartMonth)[2] });
  const [components, setComponents] = useState<ComponentRow[]>([]);
  type MRow = { volume: string; futuresMonth: string; notes: string };
  const mkRows = (fy: string) =>
    Object.fromEntries(fiscalYearMonths(fy, fyStartMonth).map((m) => [m, { volume: "", futuresMonth: suggestFuturesMonth(commodityConfig, m), notes: "" }]));
  ...
  const sharedTotalPerBu = components.reduce((sum, r) => {
    ...
    return sum + (r.unit === "$/bu" ? val : val / commodityConfig.bushelsPerMt);
  }, 0);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const lines = months.filter((m) => parseFloat(rows[m]?.volume) > 0).map((m) => {
      ...
      const mt = bu / commodityConfig.bushelsPerMt;
    ...
    setSubmitting(true);
    try {
      await api.post(`${apiBase}/budget/bulk`, lines);
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="space-y-1">
          ...
          <select value={config.commodityCode} onChange={(e) => setConfig((c) => ({ ...c, commodityCode: e.target.value }))}
            className="w-full bg-input-bg border border-b-input text-primary rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-1 focus:ring-focus">
            {getCommodityOptions(commodityConfig).map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
          </select>
        </div>
        <div className="space-y-1">
