...
export default function ReceiptsPage() {
  ...
  const { receipts, isLoading, mutate } = useReceipts(slug, siteFilter || undefined);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing) {
        await api.put(`${apiBase}/receipts/${editing.id}`, payload);
        toast("Receipt updated", "success");
      } else {
        await api.post(`${apiBase}/receipts`, payload);
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    try {
      await api.delete(`${apiBase}/receipts/${deleteTarget.id}`);
