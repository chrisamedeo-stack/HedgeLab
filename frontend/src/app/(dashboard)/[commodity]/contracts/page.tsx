...
function ContractForm({ editing, onSaved, onCancel }: {
  ...
  const { slug, apiBase } = useCommodity();
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (editing) {
        await api.put(`${apiBase}/contracts/${editing.id}`, payload);
        toast("Contract updated", "success");
      } else {
        await api.post(`${apiBase}/contracts`, payload);
...
function LockBasisPanel({ contract, onDone }: { contract: PhysicalContractResponse; onDone: () => void }) {
  ...
  async function handleLock(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await api.patch(`${apiBase}/contracts/${contract.id}/lock-basis`, {
        basisPerBu: basisVal,
    ...
    <form onSubmit={handleLock} className="px-6 py-4 bg-main/50 border-b border-b-default space-y-3">
      ...
      <div className="flex items-end gap-3 flex-wrap">
        ...
        <div className="space-y-1">
          ...
          <input type="text" placeholder="e.g. ZCN26" value={futures} onChange={(e) => setFutures(e.target.value)}
            className="w-28 bg-input-bg border border-b-input text-primary rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-1 focus:ring-focus placeholder:text-ph" />
        </div>
...
function ContractRow({ contract, onRefresh, onEdit, onDelete }: {
  ...
  const { apiBase, config } = useCommodity();
  ...
  async function handleCancel() {
    setCancelling(true);
    try {
      await api.post(`${apiBase}/contracts/${contract.id}/cancel`, {});
...
export default function ContractsPage() {
  ...
  const { contracts, isLoading, mutate } = useContracts(slug, filterSite || undefined);
  ...
  async function handleDeleteConfirm() {
    ...
    setDeleting(true);
    try {
      await api.delete(`${apiBase}/contracts/${deleteTarget.id}`);
