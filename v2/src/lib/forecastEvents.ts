...
on(EventTypes.MTM_CALCULATED, "forecast", async (event: KernelEvent) => {
  console.log(
    `[forecast] MTM_CALCULATED — scenario baselines may need refresh`
  );
});

on(EventTypes.POSITION_ALLOCATED, "forecast", async (event: KernelEvent) => {
  console.log(
    `[forecast] POSITION_ALLOCATED — what-if and coverage scenarios may be stale`
  );
});

on(EventTypes.POSITION_DEALLOCATED, "forecast", async (event: KernelEvent) => {
  console.log(
    `[forecast] POSITION_DEALLOCATED — what-if and coverage scenarios may be stale`
  );
});

on(EventTypes.POSITION_ROLLED, "forecast", async (event: KernelEvent) => {
  console.log(
