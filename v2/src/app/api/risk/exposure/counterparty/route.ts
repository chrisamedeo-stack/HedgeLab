...
import { getExposureByCounterparty } from "@/lib/riskService";
...
export async function GET(request: Request) {
  try {
    ...
    const exposure = await getExposureByCounterparty(orgId);
    return NextResponse.json(exposure);
  } catch (err) {
    if (err instanceof PluginNotEnabledError) {
      return NextResponse.json({ error: err.message }, { status: 403 });
    }
    console.error("[risk/exposure/counterparty] GET error:", err);
